...
void removeParagem(Carreira *headCar, Paragem *headPar, Ligacao *headLig);
Ligacao *uneLigacoes(Ligacao *headLig, Ligacao *ligacao1, Ligacao *ligacao2);
...
int main() {
    ...
    while ((comando = getchar()) != 'q') {
        if (comando == 'c')
            ...
            if ((s = verificaExisteCarreira(_carreiras)) != NULL) {
                _ligacoes = removeCarreiraLigacao(_ligacoes, s);
                _paragens = removeCarreiraParagem(_paragens, s);
                _carreiras = removeCarreiraCarreira(_carreiras, s);
                free(s);
            }
        } else if (comando == 'e') {
            removeParagem(_carreiras, _paragens, _ligacoes);
        }
        else if (comando == 'a') {
...
Carreira *removeCarreiraCarreira(Carreira *head, char *nomeCarreira) {
    ...
    Carreira *auxCarreira = head, *prev;
    NodeLigacao *auxNodeLigacao;
    if (head == NULL)
        return NULL;
    if (!strcmp(head->nome, nomeCarreira)) {
        auxCarreira = head->next;
        while (head->ligacoes != NULL) {
            auxNodeLigacao = head->ligacoes;
            head->ligacoes = head->ligacoes->next;
            free(auxNodeLigacao);
        }
        free(head->nome);
        free(head);
...
char *verificaExisteParagem(Paragem *head) {
    ...
    printf("%s: no such line.", nomeParagem);
    free(nomeParagem);
    return NULL;
...
void removeParagem(Carreira *headCar, Paragem *headPar, Ligacao *headLig) {
    ...
    Carreira *auxCarreira = headCar;
    NodeLigacao *auxNodeLigacao = NULL, *tempNodeLigacao1, *tempNodeLigacao2;
    char *nomeParagem;
    lePalavra(&nomeParagem);
    while (auxCarreira != NULL) {
        while (auxCarreira->ligacoes != NULL) {
            if (!strcmp(auxCarreira->ligacoes->ligacao->origem->nome, nomeParagem)) {
                tempNodeLigacao1 = auxCarreira->ligacoes;
                auxCarreira->origem = tempNodeLigacao1->ligacao->destino;
                auxCarreira->ligacoes = auxCarreira->ligacoes->next;
                auxCarreira->custoTotal -= tempNodeLigacao1->ligacao->custo;
                auxCarreira->duracaoTotal -= tempNodeLigacao1->ligacao->duracao;
                if (auxCarreira->ligacoes == NULL)
                    auxCarreira->numLigacao -= 2;
                else
                    auxCarreira->numLigacao--;
                free(tempNodeLigacao1);
            } else {
                auxNodeLigacao = auxCarreira->ligacoes;
                break;
            }
        }
        while (auxNodeLigacao != NULL) {
            if (!strcmp(auxNodeLigacao->ligacao->destino->nome, nomeParagem)) {
                if (auxNodeLigacao->next == NULL) {
                    tempNodeLigacao1 = auxNodeLigacao;
                    auxCarreira->destino = tempNodeLigacao1->ligacao->origem;
                    auxCarreira->custoTotal -= tempNodeLigacao1->ligacao->custo;
                    auxCarreira->duracaoTotal -= tempNodeLigacao1->ligacao->duracao;
                    if (auxNodeLigacao->prev == NULL)
                        auxCarreira->numLigacao -= 2;
                    else
                        auxCarreira->numLigacao--;
                } else {
                    tempNodeLigacao1 = auxNodeLigacao->ligacao;
                    tempNodeLigacao2 = auxNodeLigacao->next->ligacao;
                    auxNodeLigacao = uneLigacoes(headLig, tempNodeLigacao1, tempNodeLigacao2);
                    tempNodeLigacao1->prev->next = auxNodeLigacao; 
                }
            }
        }
        auxCarreira = auxCarreira->next;
...
Ligacao *uneLigacoes(Ligacao *headLig, Ligacao *ligacao1, Ligacao *ligacao2) {
    ...
    Ligacao *newLigacao = (Ligacao*) malloc(sizeof(Ligacao));
    newLigacao->carreira = ligacao1->carreira;
    newLigacao->origem = ligacao1->origem;
    newLigacao->destino = ligacao2->destino;
